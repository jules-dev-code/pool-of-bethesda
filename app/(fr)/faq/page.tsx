...
export default function FaqPage() {
  ...
  const filtered = useMemo(() => {
    return FAQS.filter((f) => {
      ...
      const matchesQuery =
        query.trim() === "" ||
        f.questionFr.toLowerCase().includes(query.toLowerCase()) ||
        f.answerFr.toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <>
      <JsonLd data={getFaqSchema(FAQS, "fr")} />
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {filtered.map((faq) => (
                <AccordionItem key={faq.id} value={faq.id}>
                  <AccordionTrigger>{faq.questionFr}</AccordionTrigger>
                  <AccordionContent>{faq.answerFr}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          )}
      ...
      <CtaSection
        locale="fr"
        title="Une autre question ? Contactez-nous."
